...
func pullImage(data *Data, client *client.Client, authConfig *AuthConfigs, image string) error {
	...
	if err != nil {
		return fmt.Errorf("error pulling image %s: %s - tried to use %s to retrieve config auth. Config auth: %v", image, err, registry, authConfig.Configs)
	}
	defer out.Close()
...
func parseImageOptions(image string, authConfig *AuthConfigs) internalPullImageOptions {
	...
	for registry := range authConfig.Configs {
		log.Println("[DEBUG] Looking for " + registry + " in " + image)
